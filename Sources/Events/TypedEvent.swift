...
/// A protocol for events that carry typed payloads.
...
/// A generic event that carries a typed payload.
...
/// Type-safe event handler that receives typed events.
public typealias TypedEventHandler<T> = (TypedEvent<T>) -> Void

/// Extension to `EventBroadcaster` that provides type-safe event handling.
extension EventBroadcaster {
  /// Subscribes a typed handler to an event type, automatically casting events
  /// to the specified typed event.
  ///
  /// - Parameters:
  ///   - eventType: The type of events to subscribe to.
  ///   - handler: The typed handler to invoke with events.
  ...
  /// Subscribes a typed handler using an object subscriber.
  ///
  /// - Parameters:
  ///   - subscriber: The object to associate with this subscription.
  ///   - eventType: The type of events to subscribe to.
  ///   - handler: The typed handler to invoke with events.
  public func subscribe<T>(
